...
/* ---------- bande démo (Vimeo, pour garder la pleine qualité) ---------- */
// Remplacez VIMEO_ID par l'identifiant numérique de votre vidéo Vimeo
// (visible dans son URL : vimeo.com/XXXXXXXXX). Réglages recommandés côté
...
(function initReelModal(){
  ...
  if(!heroVideo || !heroPlay || !videoModal || !videoEmbed || !closeBtn) return;

  function openReel(){
    videoEmbed.innerHTML = `
      <iframe src="https://player.vimeo.com/video/${VIMEO_ID}?autoplay=1&title=0&byline=0&portrait=0"
  ...
  function closeReel(){
    ...
    unlockPageScroll();
    videoEmbed.innerHTML = ''; // retire l'iframe : coupe net la lecture, pas juste une pause
    heroVideo.play().catch(() => {});
  }

  heroPlay.addEventListener('click', openReel);
  closeBtn.addEventListener('click', closeReel);
  videoModal.addEventListener('click', e => { if(e.target === videoModal) closeReel(); });
  document.addEventListener('keydown', e => {
    if(videoModal.classList.contains('open') && e.key === 'Escape') closeReel();
  });
})();
