...
// Charge la version desktop (paysage) ou mobile (portrait, plus légère)
// selon la largeur d'écran. Les deux fichiers doivent exister dans assets/
// (voir data-src-desktop / data-src-mobile dans index.html).
(function initHeroVideo(){
  const video = document.getElementById('heroVideo');
  if(!video) return;
  const isSmallScreen = window.matchMedia('(max-width: 48em)').matches;
  const src = isSmallScreen ? video.dataset.srcMobile : video.dataset.srcDesktop;
  if(src){
    video.src = src;
    video.load();
    video.play().catch(() => {}); // autoplay peut être bloqué avant interaction, sans gravité
  }
})();
...
function stillsMarkup(p, i){
  ...
  return p.stills.map((s, idx) => `
    <button data-project="${i}" data-index="${idx}">
      <img src="${s.src}"
           ${s.srcset ? `srcset="${s.srcset}" sizes="(max-width: 820px) 50vw, 30vw"` : ''}
